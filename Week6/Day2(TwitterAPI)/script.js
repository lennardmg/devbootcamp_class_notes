...
// the iife is not getting invoked
...
(function () {
    ...
    // First the ajax request needs to happen, in order to create the actual html <a>'s (line 100-103)

    $.ajax({
        url: "./links.json",
        method: "GET",
        success: function (dataResponse) {
            console.log("im in the ajax request, data is: ", dataResponse);
            var myHtml = "";
            console.log("myHTML", myHtml);
            for (var i = 0; i < dataResponse.length; i++) {
                console.log("dataResponse[i]", dataResponse[i]);
                var link = 
                    "<a href=" + dataResponse[i].href + ">" + dataResponse[i].description + "</a>"
        ...
        error: function (err) {
            console.log("an error occured in ajax request to links.json ", err);
        },
    });

    // then the actual function needs to be called, which creates the whole moving (moveHeadlines) and 
    // the events (mouseenter / -leave)
